...
project_path = os.path.join(pathlib.Path(__file__).parent.resolve(), "../..")
...
for bcdline in sys.stdin:
    ...
    if barcodegroup != barcode:
        if barcodegroup:
            cf.close()
            _ = subprocess.run(f'''{os.path.join(project_path, "Rearrangement", "build", "rearrangement")} <{fq1}.countfile 3<{fq1}.reference -u -3 -v -9 -s0 -6 -s1 4 -s2 2 -qv -9 | {os.path.join(project_path, "tools", "correct_micro_homology.AWK")} -- {ext1up} NGG {ext2up} NGG {len(ref1)} | tee -a {fq1}.alg | awk -v OFS="\t" -v barcode={barcodegroup} -v cut1={ext1up} -v cut2={len(ref1)+ext2up} 'NR%3==1{{print barcode, $0, cut1, cut2}}' >> {fq1}.table; echo >> {fq1}.alg''', shell=True, check=True)
