...
newRange <- function(old_vals, new_min = 0, new_max = 1) {
  ...
  new_vals = (((old_vals - old_min) * (new_max - new_min)) /
    (old_max - old_min)) +
    new_min
  new_vals
}
...
geom_logo <- function(
  ...
  logo_layer = layer(
    stat = 'identity',
    data = data,
    mapping = aes(x = x, y = y, fill = group, group = group_by),
    geom = 'polygon',
    position = 'identity',
    show.legend = NA,
    inherit.aes = F,
  ...
  list(
    logo_layer,
    scale_x_continuous(breaks = breaks_fun, labels = identity),
    ylab(y_lab),
    xlab(''),
    colscale_opts,
    guides_opts,
...
#' @param scales Facet scales, see facet_wrap
#' @param ncol Number of columns, works only when \code{facet='wrap'}, see facet_wrap
