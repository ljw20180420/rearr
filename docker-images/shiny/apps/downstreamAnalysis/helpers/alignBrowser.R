arrangeInsertion <- function(query, insertion, insertionCollapse) {
  ...
  for (i in seq_len(length(insertionCollapse))) {
    ...
    if (!insertionAdded) {
      insertionLines <- c(
        insertionLines,
        sprintf("%s%s", strrep(" ", insertionCollapse[i] - 1), insertionSeg)
      )
    }
  }
...
snpHighlight <- function(refSeg, querySeg) {
  ...
  for (i in seq_len(length(boundaries) / 2)) {
    mdStr <- sprintf(
      "%s%s<span style=\"color: red;\">%s</span>",
      mdStr,
...
getMarkdownFromAlign <- function(algTibble) {
  ...
  for (i in seq_len(nrow(algTibble))) {
    allMd[2 * i - 1] <- paste0(
      sprintf(
        "<span style=\"background-color: lightgrey;\">%10s%s",
        algTibble$refId[i],
        substr(algTibble$refNoGap[i], 1, algTibble$cut1[i] - 1)
      ),
      sprintf(
        '<span style=\"letter-spacing: -0.3em;\">%s|</span>',
      ...
      sprintf(
        '<span style=\"color: blue; letter-spacing: -0.3em;\">%s</span><span style=\"letter-spacing: -0.3em;\">|</span>',
        substr(
          algTibble$refNoGap[i],
    ...
    if (insertion[1] == 1) {
      ...
      queryMd <- sprintf(
        "%s%s<span style=\"letter-spacing: -0.3em;\">%s</span>",
        queryMd,
        sprintf("%9s", substr(countStr, 1, countLen - 1)),
    ...
    for (j in seq_len(length(insertion))) {
      ...
      if (
        lastBase != toupper(substr(algTibble$refNoGap[i], refEnd, refEnd)) &&
          lastBase != "-"
      ) {
