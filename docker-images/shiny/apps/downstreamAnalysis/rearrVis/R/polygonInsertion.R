...
getPolyInsTibble <- function(algTibble) {
  ...
  for (i in seq_along(insRegs)) {
    ...
    polyInsTibble$insPos2[[i]] <- insPos[mask2] -
      algTibble$ref1Len[i] -
      algTibble$cut2[i]
...
getPolyXY <- function(polyInsTibble, mode) {
  ...
  if (mode == "down") {
    polyX[seq(3, length(polyX), 4)] <- polyX[seq(3, length(polyX), 4)] +
      polyInsTibble$insLen
  } else if (mode == "up") {
    polyX[seq(3, length(polyX), 4)] <- polyX[seq(3, length(polyX), 4)] -
      polyInsTibble$insLen
  } else {
  ...
  polyY[2:3 + rep(seq(0, length(polyY) - 1, 4), each = 2)] <- rep(
    polyInsTibble$count,
    each = 2
  )
