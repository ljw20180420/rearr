...
ref = manimDNA.generate_random_DNA(18)
ref = ref[:13] + "GG" + ref[15:]
region1 = manimDNA.indel_DNA(manimDNA.SNP_DNA(ref[2:-2], 0.05), 0.05)
region2 = manimDNA.indel_DNA(manimDNA.SNP_DNA(ref[2:-2], 0.05), 0.05)
query = (
    manimDNA.generate_random_DNA(2)
    + region1
    + manimDNA.generate_random_DNA(32 - len(region1) - len(region2))
    + region2
    + manimDNA.generate_random_DNA(2)
)
...
class Forward(Scene):
    def construct(self):
        ...
        self.play([FadeIn(C_scores1[j]) for j in range(3, len(query) + 1)])
