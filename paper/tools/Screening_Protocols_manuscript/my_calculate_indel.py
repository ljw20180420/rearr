...
def calc_mle(total_reads, indel_counts, background):
    ...
    distrib = binom.pmf(
        indel_counts - indel_range, total_reads - indel_range, background
    )
    if len(distrib) == 0:
        mle_freq = 0
    else:
    ...
    z = 1.96
    upper_bound = (
        total_reads * mle_freq
        + z**2 / 2
        + z * np.sqrt(total_reads * mle_freq * (1 - mle_freq) + z**4 / 4)
    ) / (total_reads + z**2)
    lower_bound = (
        total_reads * mle_freq
        + z**2 / 2
    ...
    with open(sample_sheet) as in_handle:
        for i, l in enumerate(in_handle):
            if len(l.strip().split(",")) < 5:
                ...
                controls.append(i)
    background_list = [float(output_text[i].split(",")[7]) / 100 for i in controls]
    ...
    with open(output_file, "w") as out_handle:
        ...
        for i, l in enumerate(output_text):
            if i in controls:
                ...
                out_handle.write(
                    "{},{},{},{}\n".format(l, mle_percentage, lower_bound, upper_bound)
                )
...
def op_ver(opcodes):
    ...
    if len(ops) % 2:
        ...
        if not (ops[0] == "d" and ops[-1] == "d" and set(ops[1::2]) == set(["e"])):
            ...
            proc_ops = [
                (x[0][0], x[3], x[4] - x[3], x[1], x[1] - x[2]) for x in opcodes[2:-2:2]
            ]
            return proc_ops
    else:
        return False
...
def indel_calc_window_hash(seq_handle, target):
    ...
    for readout in seq_handle:
        ...
        if (
            len(read) < MIN_READ_LENGTH or read.count("N") > MAX_AMBIGUOUS_BASES
        ):  # filtering for junk
            rejected_total += 1
        elif target in read:
            perf_total += 1
            ...
            for kmer in read_index:
                if (
                    read_index[kmer] is not None
                    and kmer in target_index
                    and target_index[kmer] is not None
                ):
            ...
            if len(mapping) == 0:
                ...
                index_diff = (
                    mapping[i] - i if i in mapping else None
                    for i in range(len(read) + KMER_SIZE + 1)
                )
                collapsed_dif = [
                    [k, len(list(g))] for k, g in itertools.groupby(index_diff)
                ]
...
def indel_calc_window(seq_handle, target, start_pos):
    """
    ...
    for readout in seq_handle:
        opcodes2 = difflib.SequenceMatcher(
            None, target, str(readout.seq), autojunk=False
        ).get_opcodes()  # added by ljw
        refcoor, seqcoor = [opcodes2[0][1] + start_pos, opcodes2[0][2] + start_pos], [
            opcodes2[0][3],
            opcodes2[0][4],
        ]  # added by ljw
        for i in range(1, len(opcodes2)):  # added by ljw
            refcoor.append(opcodes2[i][2] + start_pos)  # added by ljw
            seqcoor.append(opcodes2[i][4])  # added by ljw
        sys.stdout.write(
            f"{readout.name}\t"
            + "\t".join([str(i) for i in (refcoor + seqcoor)])
            + "\n"
        )  # added by ljw
        ...
        if (
            ...
            opcodes = difflib.SequenceMatcher(
                None, read, target, autojunk=False
            ).get_opcodes()
            ...
            if len(opcodes) > 3 + MAX_INDEL_MISMATCH * 2:
                ...
                if not len(opcodes) % 2:
                    opcodes = difflib.SequenceMatcher(
                        None, read, target[1:-1], autojunk=False
                    ).get_opcodes()
                ...
                if not indel_list:
                    ...
                    # miscalled bases
                    miscall = set.union(
                        set(x[2] for x in indel_list),
                        set(x[4] for x in indel_list),
                        set(x[0] for x in indel_list),
                    ) == set(["r", 1, -1])
...
def file_calc(f_name, guide_loc, target, file_type, hash_flag):
    ...
    while error_flag:  # attempt windows while above threshold
        start_pos = guide_loc[0] - window_size  # added by ljw
        target_window = target[guide_loc[0] - window_size : guide_loc[1] + window_size]
        with open(f_name, "r") as f_handle:
            if hash_flag:
                total_list = indel_calc_window_hash(
                    SeqIO.parse(f_handle, file_type), target_window
                )
            else:
                total_list = indel_calc_window(
                    SeqIO.parse(f_handle, file_type), target_window, start_pos
                )

        err_total = total_list[2]
        rejected_total = total_list[3]
        error_percentage = float(err_total) / (sum(total_list) - rejected_total) * 100
...
def prep_entry(f_name, guide, target, file_type, hash_flag):
    ...
    if guide in target:
        ...
        total_list, note = file_calc(
            f_name, find_loc(rc(guide), target), target, file_type, hash_flag
        )
    else:
        total_list = (0,) * 6
...
def whole_file_read(
    sample_sheet, file_type, output_file, hash_flag, mle, verbose, quiet
):
    """
    ...
    with open(sample_sheet) as in_handle, open(output_file, "w") as out_handle:
        ...
        for l in in_handle:
            ...
            try:
                ...
                continue
            indel_rate = float(indel_total) / (sum(total_list) - rejected_total)
            total_list_string = ",".join(str(s) for s in total_list)
            out_handle.write(
                "{},{},{},{}\n".format(sample_name, total_list_string, indel_rate, note)
            )
    if not mle:
    ...
    with open(sample_sheet) as in_handle:
        for l in in_handle:
            ...
            if sample_name.strip() == input_name.strip():
                with open(
                    SINGLE_FILE_STRUCTURE.format(input_name.strip()), "w"
                ) as out_handle:
                    guide = guide.upper().strip()
                    target = target.upper().strip()
                    total_list, note = prep_entry(
...
def combine_files(sample_sheet, file_type, output_file, mle, verbose, quiet):
    ...
    with open(sample_sheet) as in_handle, open(output_file, "w") as out_handle:
        ...
        for l in in_handle:
            sample_name, file_name, guide, target = l.strip().split(",")[:4]
            with open(
                SINGLE_FILE_STRUCTURE.format(sample_name.strip()), "w"
            ) as samp_handle:
...
if __name__ == "__main__":
    ...
    combine.add_argument(
        "-c",
        "--combine",
        help="combines files generated by individual samples",
        action="store_true",
    )
    parser.add_argument(
        "-f",
        "--fasta",
        help="reads fasta files (default is fastq)",
        action="store_true",
    )
    parser.add_argument(
        "-no-m",
