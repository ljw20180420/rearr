...
random_seq_methods.py $ref 0.02 50 > random.seq
...
mkdir -p CVS
bwa index -p CVS/ref ref.fa
bwa mem -v 3 -T -9999 CVS/ref random.fq | samtools sort -o CVS/random.bam
samtools index -b CVS/random.bam
Rscript variants.r $ref CVS/random.bam $sgstart $sglen | sort -k1,1V > CVS/random.cvs
ARRANGE_RESULTS.py CrisprVariants $sgstart <CVS/random.cvs | awk -v OFS="\t" -v cut=$(expr ${#ref} / 2) 'NF==1{print $0,cut,cut,0} NF>1{print}' >CrisprVariants.arr

rm -rf AMP; mkdir -p AMP
primer=$(cut -c $(expr $reflen / 4 - 5)-50 <<<$ref)
...
join -t $'\t' -1 2 -2 1 <(sed -nr '1~4{s/^@//; N; s/\n/\t/; p}' random.fq | sort -k2,2) <(join -t $'\t' -1 1 -2 1 <(sed -nr '1~4N; s/\n/\t/p' random.fq | sort -k1,1) <(ARRANGE_RESULTS.py amplican $(expr $reflen / 4 - 6) <AMP/alignments/alignments.txt | sort -k1,1) | cut -f2- | sort -k1,1) | cut -f2- | sort -k1,1V >amplican.arr
