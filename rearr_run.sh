...
case $input in # count duplicate reads, support fasta, fastq, and their compressions
    *.fq.gz|*.fastq.gz)
        ...
        pv -c -N "count $input" "$input" | zcat | sed -n '2~4p' | sort | uniq -c | sort -k1,1nr | gawk -v OFS="\t" '{print $2, $1}' >"$input.count";;
    *.fa.gz|*.fasta.gz)
        echo "the input is gzip fasta file"
        pv -c -N "count $input" "$input" | zcat | sed -n '2~2p' | sort | uniq -c | sort -k1,1nr | gawk -v OFS="\t" '{print $2, $1}' >"$input.count";;
    *.fq|*.fastq)
        echo "the input is fastq file"
        pv -c -N "count $input" "$input" | sed -n '2~4p' | sort | uniq -c | sort -k1,1nr | gawk -v OFS="\t" '{print $2, $1}' >"$input.count";;
    *.fa|*.fasta)
        echo "the input is fasta file"
        pv -c -N "count $input" "$input" | sed -n '2~2p' | sort | uniq -c | sort -k1,1nr | gawk -v OFS="\t" '{print $2, $1}' >"$input.count";;
esac
ref1=$2 # reference1
ref2=$3 # reference2
...
pv -c -N "align $input" "$input.count" | $project_path/Rearrangement/build/rearrangement 3<"$input.ref.$cut1.$cut2.${#ref1}" -u -3 -v -9 -s0 -6 -s1 4 -s2 2 -qv -9 | gawk -f $project_path/tools/correct_micro_homology.AWK -- "$cut1" "$NGGCCNtype1" "$cut2" "$NGGCCNtype2" ${#ref1} | tee "$input.alg.$cut1.$cut2.${#ref1}" | gawk -F "\t" -v OFS="\t" -v cut1="$cut1" -v cut2=$((${#ref1} + "$cut2")) -v total_count=$(gawk -F "\t" '{count += $2} END{print count}' $input.count) '
    BEGIN{
