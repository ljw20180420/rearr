...
read cut ext1 ext2 <<<$(awk -F "." -v OFS=" " '{print $(NF-2), $(NF-1), $NF}' <<<$algfile)
readnum=${2:-50} # read number to display (default: 50)
head -n$(($readnum * 3)) <$algfile | align_align.py $(($cut + $ext1)) $cut $(($cut + $ext1 + $ext2)) | paste - <(head -n$(($readnum + 1)) $tablefile | awk -F "\t" 'NR==1{for(i=1;i<NF;++i) if($i=="percent"){pf=i;break} print ""} NR>1{printf("%.2f%\n",$pf)}') | column -ts $'\t' | less-643/less -SNR --header 1 --no-number-headers # view the first 50 (150/3) read alignments