...
head -n$(("$readnum" * 3)) <"$algfile" | "${project_path}/tools/align_align.AWK" -- "$ref1len" "$cut1" $(("$ref1len" + "$cut2")) | paste - <(head -n$(("$readnum" + 1)) "$tablefile" | ${project_path}/gawk-5.3.0/gawk -F "\t" 'NR==1{for(i=1;i<=NF;++i) if($i=="percent"){pf=i;break} print ""} NR>1{printf("%s\n",$pf)}') | column -ts $'\t' | "${project_path}/less-643/less" -SNR --header 1 --no-number-headers # view the first 50 (150/3) read alignments