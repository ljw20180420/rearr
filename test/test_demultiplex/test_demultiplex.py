...
class TestDemultiplex(unittest.TestCase):
    def setUp(self):
        ...
        self.spliters = [
            mkstemp(dir="test/test_demultiplex", suffix=".spliter")[1] for _ in range(3)
        ]
        ...
        with open(self.rmDupFile, "w") as wd:
            ...
            for spliter in self.spliters:
                with open(spliter, "r") as fd:
                    spliters_seqs.append(fd.read().splitlines()[1::2])
            for i in range(self.noDupNum):
                ridx = rng.integers(self.spliter_num)
                r_spliter_seqs = [spliter_seqs[ridx] for spliter_seqs in spliters_seqs]
    ...
    def test_demultiplex(self):
        subprocess.run(
            f"""spliterIndices={','.join(self.spliters)} minScores={','.join(self.minScores)} demultiplex.sh {self.rmDupFile} > {self.demultiplexFile}""",
            shell=True,
            executable="/bin/bash",
        )
        self.assertTrue(
