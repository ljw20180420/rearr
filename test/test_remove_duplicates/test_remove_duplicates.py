...
class TestRemoveDuplicates(unittest.TestCase):
    def setUp(self):
        ...
        self.fastqs = [
            mkstemp(
                dir="test/test_remove_duplicates", suffix=rng.choice([".fq", ".fastq"])
            )[1]
            for _ in range(3)
        ]
        self.seq_num = 1000
        ...
        for fastq in self.fastqs:
            with open(fastq, "w") as fd:
                ...
                for i, idx in enumerate(idxs):
                    seq = seqs[idx]
                    qual = "".join(
                        [chr(q33) for q33 in rng.integers(33, 126, len(seq))]
                    )
                    fd.write(f"""@seq{i}\n{seq}\n+\n{qual}\n""")

    def test_remove_duplicates(self):
        subprocess.run(
            f"""removeDuplicates.sh {' '.join(self.fastqs)} > {self.rmDupFile}""",
            shell=True,
            executable="/bin/bash",
        )
        self.assertTrue(
