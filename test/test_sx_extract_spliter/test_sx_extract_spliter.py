...
class TestSxExtractSpliter(unittest.TestCase):
    ...
    def test_sx_extract_spliter(self):
        for csvfile, spliter1, spliter2 in zip(
            ...
            self.assertTrue(
                filecmp.cmp(spliter1, f"{csvfile}.target.fa", shallow=False)
            )
            self.assertTrue(filecmp.cmp(spliter2, f"{csvfile}.pair.fa", shallow=False))
