...
for (i in seq_len(lineNum / 3)) {
  if (nchar(reflines[i]) != nchar(querylines[i])) {
    print(i)
    print(reflines[i])
    print(querylines[i])
    break
...
base_pos_df |>
  mutate(part = factor(
    ifelse(rpos == 0,
      "updangle", 
      ifelse(lpos == nchar(ref1) + nchar(ref2),
        "downdangle",
        ifelse(lpos == nchar(ref1) & rpos == nchar(ref1),
          "randomins",
          ifelse(lpos < nchar(ref1),
            "ref1",
            "ref2"
          )
        )
      )
    ), levels = c("updangle", "ref1", "randomins", "ref2", "downdangle"))) |>
