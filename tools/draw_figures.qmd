...
```{python set parameters for python, include = FALSE}
algfile = r.algfile
...
cut2 = r.cut2
lineNum = r.lineNum
with open(algfile, "r") as pf:
    _ = pf.readline()
    ref1 = pf.readline().rstrip().upper().replace('-', '')
ref2 = ref1[ref1len:]
ref1 = ref1[:ref1len]
ref1vec = list(ref1)
ref2vec = list(ref2)
...
```{python get read counts, include = FALSE}
with os.popen(f'''awk -F "\t" 'NR % 3 == 1{{print $2}}' {algfile}''', "r") as pf:
    counts = numpy.reshape(numpy.loadtxt(pf, dtype = numpy.int32), (-1, 1))
```

```{python calculate base frequency, include = FALSE}
with os.popen(f'''awk -F "\t" 'NR % 3 != 1{{print $0}}' {algfile}''', "r") as pf:
    lines = pf.readlines()
basefreq, alphabet = numpy.zeros((5, len(ref1) + len(ref2)), dtype = numpy.int64), numpy.reshape(numpy.frombuffer(b"ACGT-", dtype = numpy.int8), (5, 1))
for i in range(0, len(lines), 2):
    querybases = numpy.frombuffer(lines[i + 1].rstrip().encode(), dtype = numpy.int8)[numpy.frombuffer(lines[i].rstrip().encode(), dtype = numpy.int8) != ord('-')]
    basefreq += (querybases == alphabet) * counts[i // 2]
```

```{python}
with os.popen(f'''awk -F "\t" 'NR % 3 == 2{{print $0}}' {algfile}''', "r") as pf:
    lines = pf.readlines()
insertcount = numpy.zeros(len(ref1) + len(ref2) + 1, dtype = numpy.int64)
for i in range(len(lines)):
    mask = numpy.frombuffer(lines[i].rstrip().encode(), dtype = numpy.int8) != ord('-')
    insertcount += numpy.bincount(numpy.cumsum(mask)[mask == False], minlength=len(insertcount)) * counts[i]

with os.popen(f'''awk -F "\t" 'NR % 3 != 1{{print $0}}' {algfile}''', "r") as pf:
    lines = pf.readlines()
MSDfreq = numpy.zeros((3, len(ref1) + len(ref2)), dtype = numpy.int64)
for i in range(0, len(lines), 2):
    refbases = numpy.frombuffer(lines[i].rstrip().encode(), dtype = numpy.int8)
    mask = (refbases != ord('-'))
    querybases = numpy.frombuffer(lines[i + 1].rstrip().encode(), dtype = numpy.int8)[mask]
    refbases = refbases[mask]
    MSDfreq[0, :] += (refbases == querybases)
    MSDfreq[1, :] += numpy.logical_and(querybases != ord('-'), refbases != querybases)
    MSDfreq[2, :] += (querybases == ord('-'))
...
for i in range(0, len(lines), 2):
    refbases.append(numpy.round(poly(numpy.frombuffer(lines[i].rstrip().upper().encode(), dtype = numpy.int8))).astype(numpy.int8))
    querybases.append(numpy.round(poly(numpy.frombuffer(lines[i + 1].rstrip().encode(), dtype = numpy.int8))).astype(numpy.int8))
...
tibble(count = c(py$basefreq), pos = rep(seq(ncol(py$basefreq)) - 0.5, each = nrow(py$basefreq)), base = rep(c("A", "C", "G", "T", "-"), times = ncol(py$basefreq))) |>
  ...
  geom_step(aes(pos - cut1, count), data = tibble(pos = 0:(nchar(py$ref1)+nchar(py$ref2)), count = py$insertcount) |> filter(pos >0, pos < ref1len), color = "black", direction = "mid") +
...
tibble(count = c(py$basefreq), pos = rep(seq(ncol(py$basefreq)) - 0.5, each = nrow(py$basefreq)), base = rep(c("A", "C", "G", "T", "-"), times = ncol(py$basefreq))) |>
  ...
  geom_col(aes(fill = base)) +
  geom_step(aes(pos - ref1len - cut2, count), data = tibble(pos = 0:(nchar(py$ref1)+nchar(py$ref2)), count = py$insertcount) |> filter(pos > ref1len, pos < nchar(py$ref1)+nchar(py$ref2)), color = "black", direction = "mid") +
...
tibble(count = py$insertcount[c(1, ref1len + 1, nchar(py$ref1) + nchar(py$ref2) + 1)], part = c("updangle", "randomins", "downdangle")) |>
...
microhomo <- matrix(as.integer(rep(py$ref1vec, time = length(py$ref2vec)) == rep(py$ref2vec, each = length(py$ref1vec))), nrow = length(py$ref1vec))
